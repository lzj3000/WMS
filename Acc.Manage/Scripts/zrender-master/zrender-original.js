...
width         : {number},  // 必须，宽度
height        : {number},  // 必须，高度
direction     : {string},  // 默认left, left | up | down | right
brushType     : {string},  // 默认为fill，绘画方式
// fill(填充) | stroke(描边) | both(填充+描边)
color         : {color},   // 默认为'#000'，填充颜色，支持rgba
strokeColor   : {color},   // 默认为'#000'，描边颜色（轮廓），支持rgba
...
define(
    'zrender/shape/arrow', ['require', './base', '../shape'], function (require) {
        function Arrow() {
            this.type = "arrow";
        }
        ...
        Arrow.prototype = {
            ...
            */
            buildPath: function (ctx, style) {
                //方向
                var d = style.direction || "left";

                switch (d) {
                    case "right":
                        var w = style.width * 0.6;
                        var h = style.height / 4;
                        ctx.moveTo(style.x, style.y + h);
                        ctx.lineTo(style.x + w, style.y + h);
                        ctx.lineTo(style.x + w, style.y);
                        ctx.lineTo(style.x + style.width, style.y + h * 2);
                        ctx.lineTo(style.x + w, style.y + style.height);
                        ctx.lineTo(style.x + w, style.y + style.height - h);
                        ctx.lineTo(style.x, style.y + style.height - h);
                        ctx.lineTo(style.x, style.y + h);
                        return;
                    case "left":
                        var w = style.width * 0.4;
                        var h = style.height / 4;
                        ctx.moveTo(style.x, style.y + h * 2);
                        ctx.lineTo(style.x + w, style.y);
                        ctx.lineTo(style.x + w, style.y + h);
                        ctx.lineTo(style.x + style.width, style.y + h);
                        ctx.lineTo(style.x + style.width, style.y + h * 3);
                        ctx.lineTo(style.x + w, style.y + h * 3);
                        ctx.lineTo(style.x + w, style.y + style.height);
                        ctx.lineTo(style.x, style.y + h * 2);
                        return;
                    case "up":
                        var h = style.height * 0.4;
                        var w = style.width / 4;
                        ctx.moveTo(style.x + w * 2, style.y);
                        ctx.lineTo(style.x + style.width, style.y + h);
                        ctx.lineTo(style.x + w * 3, style.y + h);
                        ctx.lineTo(style.x + w * 3, style.y + style.height);
                        ctx.lineTo(style.x + w, style.y + style.height);
                        ctx.lineTo(style.x + w, style.y + h);
                        ctx.lineTo(style.x, style.y + h);
                        ctx.lineTo(style.x + w * 2, style.y);
                        return;
                    case "down":
                        var h = style.height * 0.6;
                        var w = style.width / 4;
                        ctx.moveTo(style.x + w, style.y);
                        ctx.lineTo(style.x + w * 3, style.y);
                        ctx.lineTo(style.x + w * 3, style.y + h);
                        ctx.lineTo(style.x + style.width, style.y + h);
                        ctx.lineTo(style.x + w * 2, style.y + style.height);
                        ctx.lineTo(style.x, style.y + h);
                        ctx.lineTo(style.x + w, style.y + h);
                        ctx.lineTo(style.x + w, style.y);
                        return;
...
{
// 基础属性
shape  : 'hierarchy',       // 必须，shape类标识，需要显式指定
id     : {string},       // 必须，图形唯一标识，可通过zrender实例方法newShapeId生成
zlevel : {number},       // 默认为0，z层level，决定绘画在哪层canvas中
invisible : {boolean},   // 默认为false，是否可见
...
*/
    ...
    define(
    'zrender/shape/ladder', ['require', './base', '../shape'], function (require) {
        function Ladder() {
            this.type = "ladder";
        }
        ...
        Ladder.prototype = {
            ...
            */
            buildPath: function (ctx, style) {
                //层次
                var r = style.rows || 0;  //行    
                if (r > 0) {
                    var tr = (r > 0 ? style.height / r : 0);
                    for (var j = 0; j < r; j++) {

                        ctx.rect(style.x, style.y + j * tr, style.width, tr - 1);
                        if (j < r - 1) {
                            ctx.strokeRect(style.x, style.y + j * tr, 1, 1);
                        }
                    }
                } else {
        ...
        var shape = require('../shape');
        shape.define('ladder', new Ladder());
        ...
        */
        function ZRender(id, dom, params) {
            ...
            */
            ...
            /**
            * 获取所有图形集合  lsc  2014-2-20
            */
            self.getAllShapes = function () {
                return storage.getAllShapes();
            }
        }
        ...
        */
        function Storage(shape) {
            ...
            */
            ...
            self.getAllShapes = getAllShapes;
        }
